...
export default function PaymentHistoryPage() {
  ...
  return (
    <DashboardLayout title="Payment History" subtitle="View all your payment transactions">
      <PageContent user={user} />
    </DashboardLayout>
  );
}

function PageContent({ user }: { user: any }) {
  const [payments, setPayments] = useState<Payment[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState('');
      ...
      >
        {isLoading ? (
          ...
          <div className="bg-card border border-border/50 rounded-xl p-8 md:p-12 text-center">
            <h3 className="text-lg md:text-xl font-semibold text-foreground mb-2">No payments found yet</h3>
            <p className="text-muted-foreground text-sm md:text-base">Upgrade your account to get started.</p>
          </div>
        ) : (
              ...
              <table className="w-full">
                <thead className="bg-muted border-b border-border">
                  <tr>
                      ...
                      <th className="px-6 py-4 text-left text-sm font-semibold text-gray-300">Months</th>
                  ...
                  <tbody>
                    {payments.map((payment, index) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 text-sm text-white font-semibold">Rs {payment.amount}</td>
                        <td className="px-6 py-4 text-sm text-gray-400">{payment.payment_method}</td>
                        <td className="px-6 py-4 text-sm text-gray-400">{payment.sender_number}</td>
                        <td className="px-6 py-4 text-sm text-gray-400">{payment.receiver_number}</td>
                        <td className="px-6 py-4 text-sm text-white">{payment.months}</td>
                        <td className="px-6 py-4 text-sm">
                          <span className={`inline-block px-3 py-1 rounded-full border text-xs font-semibold ${STATUS_COLORS[payment.status]}`}>
                            {payment.status}
                          </span>
                        </td>
                        <td className="px-6 py-4 text-sm">
                ...
                {payments.map((payment, index) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-white font-semibold">{payment.date}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-gray-400 text-sm">Amount</span>
                      <span className="text-white font-semibold">Rs {payment.amount}</span>
                    </div>
                    <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`px-3 py-1 rounded-full border text-xs font-semibold ${STATUS_COLORS[payment.status]}`}>
                        {payment.status}
                      </span>
                    </div>
                    {payment.screenshot_url && (
