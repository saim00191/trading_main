...
export default function ContactSupportPage() {
  ...
  return (
    <DashboardLayout title="Contact Support" subtitle="Get help from our support team">
      <PageContent 
        user={user}
...
function PageContent({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    //   setIsSuccess(true);
    //   setFormData({ subject: '', category: 'Technical Issue', message: '', priority: 'Medium' });
    // } catch (err) {
          ...
          {!isSuccess ? (
            ...
            <div className="bg-card border border-border/50 rounded-xl p-4 md:p-6 shadow-lg hover:border-primary/30 transition-colors">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {/* Priority */}
                <div>
                  <label className="block text-xs md:text-sm font-semibold text-foreground mb-2">Priority *</label>
                  <select
                    name="priority"
            ...
            >
              ...
              >
                <CheckCircle2 size={32} className="md:!size-[40px] text-success" />
              </motion.div>

              <h2 className="text-2xl md:text-3xl font-bold text-foreground mb-3 md:mb-4">Submitted Successfully!</h2>
              <div className="bg-card border border-border/50 rounded-xl p-4 md:p-6 text-muted-foreground space-y-3 md:space-y-4">
                <p className="text-sm md:text-base">Thank you for reaching out. Our support team will review your request shortly.</p>
                <p className="text-sm md:text-base">
                  We will contact you directly at <span className="text-primary font-semibold">{user?.email}</span> as soon as possible.
                </p>
                <p className="text-xs md:text-sm">Your trading journey matters to us.</p>
              </div>
